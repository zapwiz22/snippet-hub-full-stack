...
const MainSnippet = () => {
  ...
  useEffect(() => {
    const fetchSnippet = async () => {
      ...
      // if not stored in the atom then only fetch
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        if (response.ok) {
          ...
          const targetSnippet = transformedSnippets.find((s) => s.id === id);
          if (targetSnippet) {
    ...
    fetchSnippet();
  }, [id, authData.userId, authData.token, snippets, setSnippets]);
  ...
  return (
    <div className="m-5 bg-white shadow-md p-8 rounded-lg">
      ...
      {snippet.contentType === "file" ? (
        ...
        <p className="bg-gray-100 p-4 rounded text-sm overflow-auto whitespace-pre-wrap">
          {snippet.content}
        </p>
      )}
