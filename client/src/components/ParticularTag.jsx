...
const ParticularTag = () => {
  ...
  const handleMoveCard = useCallback(
    (dragIndex, hoverIndex) => {
      const tagSnippetIds = tagSnippets.map((snippet) => snippet.id);
      const fullSnippetsIndices = tagSnippetIds.map((id) =>
    ...
    [tagSnippets, snippets, setSnippets, debounceReorderSave]
  );

  if (loading) {
    return (
      <div className="col-span-10 m-5 flex justify-center items-center">
        <div className="text-purple-600">Loading snippets for "{tag}"...</div>
      </div>
    );
  }
      ...
      <div className="col-span-10 m-5">
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold text-gray-700 mb-2">
            No snippets found for tag:{" "}
            <span className="font-bold text-purple-600">{tag}</span>
          </h3>
          <p className="text-gray-500">
            Try adding some snippets with this tag
          </p>
        </div>
      </div>
    );
